...
const normalReport = async (req, res) => {
  try {
    ...
    responsible =
      responsible && responsible.length > 0 ? responsible.split(",") : [];
    ...
    const data = await fetchPaginatedData(
      ...
      {
        ...(project.length > 0 && { GROUP_ID: project }),
        ...(creator.length > 0 && { CREATED_BY: creator }),
        ...(tags.length > 0 && { TAGS: tags }),
        ...(responsible.length > 0 && { ACCOMPICE: responsible }),
      },
      ["ID"]
    );
    ...
    const time = await fetchElapsedTimeData(
      `${process.env.ROOT_URL}/task.elapseditem.getlist`,
      {
        ...
        FILTER:{
          TASK_ID: taskIds,
          ">=CREATED_DATE": startDate, // Start date should be the lower bound
          "<=CREATED_DATE": `${endDate} 23:59:59`, // End date should be the upper bound with a time
        }
